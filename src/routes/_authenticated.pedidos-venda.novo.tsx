...
function NewOrderPage() {
  ...
  async function handleCreateOrder() {
    ...
    if (!clientId || items.length === 0) {
      toast.error("Cliente e pelo menos um item são obrigatórios");
      return;
    }
    ...
    const input: CreateOrderInput = {
      ...
      client_snapshot: {
        ...
        fantasyName: clientName
      },
      sellerId: 0, // Será resolvido no server
      saleTypeId: saleTypeId!,
      paymentTermId: paymentTermId!,
      paymentMethodId: paymentMethodId!,
      deliver: deliver,
      ...
      items: items.map(it => ({
        ...
        unit: it.unit || "UN",
        manualUnitPrice: it.manualPrice ? it.appliedUnitPrice : undefined
      })),
    ...
    if (result.ok) {
      ...
      toast.error("Erro ao criar pedido", { description: result.error?.message });
      setSubmissionStatus("failed");
    }
  }

  async function handleUpdateOrder() {
    if (submissionStatus === "submitting" || submissionStatus === "created" || !erpOrderNumber) return;
    ...
    const result = await updateErpOrder({
